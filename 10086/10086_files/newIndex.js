    ...
    index: {
        ...
        doSuccess: function (res) {
            if (res.result === '000') {
                ...
                $('.data_time').text(res.currentTime).css('color','#3e62ff');
                switch (res.roamingCities_15) {
                    case '-99':
                        $('.res_data_t1').text('您好！近0-14天您可能未使用过移动通讯业务，暂时无法提供行程查询服务。');
                        $('.res_data_1>.res_data_box').text('');
                        break;
                    case 'u14':
                ...
                switch (res.roamingCities_30) {
                    case '-99':
                        if (res.roamingCities_15==='-99') {
                            $('.res_data_2').hide();
                            $('.res_data_t1').text('您好！近期您可能未使用过移动通讯业务，暂时无法提供行程查询服务。');
                        }else {
                            $('.res_data_t2').text('您好！近15-30天您可能未使用过移动通讯业务，暂时无法提供行程查询服务。');
                        }
                        $('.res_data_2>.res_data_box').text('');
                        break;
                    case 'u30':
                        $('.res_data_t2').text('由于您在网时间不足30日，无法提供15-30天内行程查询服务。');
                        $('.res_data_2>.res_data_box').text('');
                        break;
                    default:
                        $ul = $('.data2');
